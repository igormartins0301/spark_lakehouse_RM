...
class DataExtractor:
    ...
    def generate_object_key(self):
        ...
        response = self.s3_client.list_objects_v2(Bucket=self.bucket_name, Prefix=f"{self.folder_name}/{self.tablename}/{today}_")
        
        if 'Contents' in response:
            existing_files = [obj['Key'] for obj in response['Contents']]
            existing_numbers = [
                int(key.split('_')[-1].split('.')[0]) for key in existing_files if key.startswith(f"{self.folder_name}/{self.tablename}/{today}_")
            ]
